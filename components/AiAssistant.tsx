...
export const AiAssistant: React.FC<AiAssistantProps> = ({ project, onUpdateProject }) => {
  ...
  const tools: FunctionDeclaration[] = [
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          priority: { type: Type.STRING, enum: ["low", "medium", "high"], description: "Prioridade" },
          scope: { type: Type.STRING, description: "Escopo (ex: Frontend, Backend). Use null se não especificado." }
        },
        required: ["title"]
      }
    },
    {
      ...
      parameters: {
        type: Type.OBJECT,
        properties: {
          taskId: { type: Type.STRING, description: "O ID exato da tarefa" },
          newStatus: { type: Type.STRING, enum: ["todo", "in-progress", "done"], description: "Novo status" }
        },
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          content: { type: Type.STRING, description: "Conteúdo do documento em Markdown" },
          scope: { type: Type.STRING, description: "Escopo opcional" }
        },
  ...
  const executeFunction = (name: string, args: any): string => {
    ...
    if (name === 'update_task_status') {
      ...
      if (!task) return `Erro: Tarefa com ID ${args.taskId} não encontrada.`;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.error(error);
      setMessages(prev => [...prev, { role: 'model', content: "Desculpe, tive um erro ao processar sua solicitação." }]);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="fixed bottom-6 right-6 z-50 w-full max-w-[380px] h-[600px] max-h-[80vh] flex flex-col bg-base-900 border border-base-700 rounded-2xl shadow-2xl animate-in slide-in-from-bottom-5 duration-300 overflow-hidden">
          ...
          <div className="flex items-center justify-between p-4 bg-base-800 border-b border-base-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-sm text-base-text">Nexion AI</h3>
          ...
          {/* Area de Mensagens */}
          <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-base-950/50" ref={scrollRef}>
          ...
          <div className="p-3 bg-base-800 border-t border-base-700">
             ...
             <div className="mt-2 flex gap-2 justify-center">
                 <span className="text-[10px] text-base-muted flex items-center gap-1 opacity-60">
                    <Terminal size={10} /> Powered by Gemini Function Calling
                 </span>
             </div>
          </div>
        </div>
